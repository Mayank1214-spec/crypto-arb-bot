...
interface CloseAttempt {
  tradeId: string;
  // Symmetric target: profit is fixed at entry regardless of where P lands
  targetClosePrice: number;  // ideal midpoint close price for both legs
  minClosePrice: number;     // long leg must sell >= this (covers close fees)
  maxClosePrice: number;     // short leg must buy  <= this (covers close fees)
  // Method 1 — RFQ close (zero taker fee, preferred)
  rfqStatus: 'IDLE' | 'PENDING' | 'PARTIAL' | 'QUOTED' | 'EXECUTED' | 'EXPIRED';
  rfqCloseId?: string;
  rfqSellQuote?: number;     // MM quote to close long leg (sell on buy exchange)
  rfqBuyQuote?: number;      // MM quote to close short leg (buy on sell exchange)
  // Method 2 — Limit orders resting at targetClosePrice
  limitLongFillable: boolean;
  limitShortFillable: boolean;
  // Method 3 — Market fallback after timeout
  marketCloseAt: number;
  marketCloseScheduled: boolean;
  // Winner
  closedBy?: 'RFQ' | 'LIMIT' | 'MARKET';
  closed: boolean;
}
...
export class ArbitrageEngine {
  ...
  private indexBasisHistory: Map<string, number[]> = new Map();
  ...
  private closeAttempts: Map<string, CloseAttempt> = new Map(); // tradeId → CloseAttempt
  ...
  private updateDepth(exchange: string, rawData: any, type: string) {
        ...
        if (type === "snapshot") {
          ...
          if (rawData.b) {
            for (const update of rawData.b) {
              ...
              if (size === 0) {
                if (idx !== -1) priceData.bids.splice(idx, 1);
              } else {
                if (idx !== -1) priceData.bids[idx][1] = size;
                else priceData.bids.push([price, size]);
              }
            }
          ...
          if (rawData.a) {
            for (const update of rawData.a) {
              ...
              if (size === 0) {
                if (idx !== -1) priceData.asks.splice(idx, 1);
              } else {
                if (idx !== -1) priceData.asks[idx][1] = size;
                else priceData.asks.push([price, size]);
              }
            }
  ...
  private updateTicker(exchange: string, rawData: any) {
    ...
    try {
      ...
      if (underlyingPrice > 0) priceData.underlyingPrice = underlyingPrice;
  ...
  private checkArbitrage(key: string, contract: OptionContract, entries: PriceData[]) {
    ...
    if (assetBasisHistory.length > 100) assetBasisHistory.shift();
  ...
  private evaluateDualQuotes(rfq: DualRfqState) {
    ...
    if (rfq.buyQuote === undefined || rfq.sellQuote === undefined) return;
   ...
   */
  private monitorCloseOrders(key: string) {
    const entries = this.prices.get(key);
    if (!entries) return;
    const deribit = entries.find(e => e.exchange === 'Deribit');
    const bybit   = entries.find(e => e.exchange === 'Bybit');
    ...
    for (const trade of this.trades) {
      ...
      if (this.getNormalizedKey(trade.opportunity.contract) !== key) continue;

      const attempt = this.closeAttempts.get(trade.id);
      if (!attempt || attempt.closed) continue;
      ...
      if (trade.opportunity.buyExchange === 'Deribit') bestBid *= buyExData.underlyingPrice;
      ...
      if (trade.opportunity.sellExchange === 'Deribit') bestAsk *= sellExData.underlyingPrice;
  ...
  private triggerRfqClose(trade: TradeRecord, attempt: CloseAttempt) {
    if (attempt.closed || attempt.rfqStatus !== 'IDLE') return;
  ...
  private simulateRfqCloseResponse(trade: TradeRecord, attempt: CloseAttempt, leg: 'LONG' | 'SHORT') {
    if (attempt.closed || (attempt.rfqStatus !== 'PENDING' && attempt.rfqStatus !== 'PARTIAL')) return;
    ...
    if (leg === 'LONG') {
      ...
      if (trade.opportunity.buyExchange === 'Deribit') bid *= buyExData.underlyingPrice;
      ...
      if (trade.opportunity.sellExchange === 'Deribit') ask *= sellExData.underlyingPrice;
  ...
  private executeMarketClose(trade: TradeRecord, attempt: CloseAttempt) {
    ...
    let closeLongPrice: number;
    if (buyExData.bids.length > 0) {
      closeLongPrice = buyExData.bids[0][0];
      if (trade.opportunity.buyExchange === 'Deribit') closeLongPrice *= buyExData.underlyingPrice;
    } else {
      closeLongPrice = trade.opportunity.buyPrice; // already USD, no further conversion
    }

    let closeShortPrice: number;
    if (sellExData.asks.length > 0) {
      closeShortPrice = sellExData.asks[0][0];
      if (trade.opportunity.sellExchange === 'Deribit') closeShortPrice *= sellExData.underlyingPrice;
    } else {
  ...
  private finalizeClose(
    ...
    method: 'RFQ' | 'LIMIT' | 'MARKET',
    closeLongPrice: number,   // price received for selling the long leg
    closeShortPrice: number   // price paid for buying back the short leg
  ) {
    if (attempt.closed) return; // Only one method can win
    attempt.closed   = true;
    ...
    if (method !== 'RFQ') {
      ...
      if      (executionType === 'DUAL_RFQ')   closeFees = 0;
      else if (executionType === 'SINGLE_RFQ') closeFees = shortCloseFee;
      else                                      closeFees = longCloseFee + shortCloseFee;
    }
    ...
    setTimeout(() => {
      ...
      for (const [k, v] of this.activeDualRfqs)   { if (v.timestamp < cutoff) this.activeDualRfqs.delete(k); }
      for (const [k, v] of this.activeSingleRfqs) { if (v.timestamp < cutoff) this.activeSingleRfqs.delete(k); }
    }, 0);
  }
